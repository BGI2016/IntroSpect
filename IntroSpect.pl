...
Version:
	Version: 1.0.0	Date: 202204	Author: zhanglesolanin\@foxmail.com
...
##	GibbsCluster options
	-GC			gibbscluster software path (STRING)
	-GCmotif		*a specific number of clusters (e.g. 3) or an interval of partitions (e.g. 1-6) [default:1-6] (INT or INT-INT)
...
unless($ss){
    die "Provide a search space list file (.ss generated by DB2SS.pl) to perform IntroSpect, -ss <ssfile> -pep <pepfile> -prefix <prefix> -GC <GCfile>" , $usage;
}
$ss="$current_path/$ss" unless($ss=~/^\//);

unless($pep){
    die "Provide a peptides list file to perform IntroSpect, -ss <ssfile> -pep <pepfile> -prefix <prefix> -GC <GCfile>" , $usage;
}
$pep="$current_path/$pep" unless($pep=~/^\//);

unless($prefix){
    die "Provide a prefix to perform IntroSpect, -ss <ssfile> -pep <pepfile> -prefix <prefix> -GC <GCfile>" , $usage;
}

unless($GC){
    die "Provide the GibbsCluster v2.0 path to perform IntroSpect, -ss <ssfile> -pep <pepfile> -prefix <prefix> -GC <GCfile>" , $usage;
}
